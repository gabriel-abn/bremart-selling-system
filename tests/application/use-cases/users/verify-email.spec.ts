...
describe("Use Case: Verify email", () => {
  describe("Basic error handling", () => {
    it("should throw if email not found", async () => {
      const { sut } = makeSut();

      await expect(() => sut.execute({ email: "invalid_email", verificationToken: "ABC123" })).rejects.toThrow(
        "INVALID_EMAIL",
      );
    });
    it("should throw if verification token is invalid or expired", async () => {
      ...
      await expect(() => sut.execute({ email: "valid_email", verificationToken: "ABC000" })).rejects.toThrow(
        "INVALID_DATA",
      );
    });
    it("should throw if user is already verified", async () => {
      ...
      await expect(() => sut.execute({ email: "valid_email_3", verificationToken: "ABC345" })).rejects.toThrow(
